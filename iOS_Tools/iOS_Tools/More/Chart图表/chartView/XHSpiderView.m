...
@interface XHSpiderView () <XHValuePopViewDelegate>
...
- (void)drawPentagonWithContext:(CGContextRef)context { // 画五边形
    ...
    for (int i = 0 ;i < kLayerCount; i++) {
        ...
        CGContextMoveToPoint(context,
                             [self.pointArray[XHSpiderPointPositionTopMid] CGPointValue].x,
                             [self.pointArray[XHSpiderPointPositionTopMid] CGPointValue].y);
...
- (void)drawValueLineWithContext:(CGContextRef)context { // 画值区域
    ...
    CGContextMoveToPoint(context,
                         [self.valuePointArray[XHSpiderPointPositionTopMid] CGPointValue].x,
                         [self.valuePointArray[XHSpiderPointPositionTopMid] CGPointValue].y);
...
- (void)showValueWithTouchPoint:(CGPoint)touchPoint {
    ...
    XHSpiderPointPosition position = [self getTouchPositionWithTouchPoint:touchPoint];
    if (position == -1) {
        return;
    }
...
- (void)showValuePopViewWithTouchPosition:(XHSpiderPointPosition)touchPosition {
    ...
    XHValuePopView *popView = [self viewWithTag:touchPosition + kPopViewTag];
    if (popView) {
        return;
    }
    popView = [[XHValuePopView alloc] initWithStarePoint:valuePoint];
    popView.tag = touchPosition + kPopViewTag;
    [popView showInView:self titleString:title];
}

- (XHSpiderPointPosition)getTouchPositionWithTouchPoint:(CGPoint)touchPoint { // 获取点击区域
    
    for (NSInteger i = XHSpiderPointPositionTopMid; i < 5; i++) {
...
- (void)onTitleLableClick:(UITapGestureRecognizer *)sender {
    ...
    if (index > XHSpiderPointPositionTopLeft || index < 0) {
        return;
    }
